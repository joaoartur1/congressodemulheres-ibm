...
-- Pedidos de camisa só são aceitos até 12/08/2026 23:59 (horário de
-- Brasília). Depois disso o formulário já esconde a opção (ver
-- src/lib/config.ts PRAZO_CAMISAS), mas o bloqueio de verdade tem que
-- estar aqui — alguém podia chamar a API direto ignorando o front-end.
...
declare
  ...
  v_prazo_camisas timestamptz := '2026-08-12 23:59:59-03'; -- manter sincronizado com PRAZO_CAMISAS
...
begin
  ...
  if jsonb_array_length(p_camisas) > 0 and now() > v_prazo_camisas then
    raise exception 'As vendas de camisa encerraram no dia 12/08 às 23h59';
  end if;
